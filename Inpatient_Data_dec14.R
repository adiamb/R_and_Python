...
ggplot(filter(inpat_agg, Provider_State == "CA"), aes(factor(DRG_Definition), Average_Total_Payments))+geom_bar(stat = "identity")+theme(axis.text.x = element_text(angle = 90, size = 10, face = "bold"), axis.text.y = element_text(size = 8, face = "bold"))+coord_flip()

###build a glm model to predict prices in the large dataset given the condition and the referral region
model1=glm(Average_Total_Payments ~ DRG_Definition+Hospital_Referral_Region_Description, data = inpatient) #family = "gaussian")
test=inpatient[15000:15010, 1:ncol(inpatient), with = F] ##select a test row from the main dataset
test$Average_Total_Payments = 0 ##assign zero to the output variable
predict(model1, test)
